...
class WebIngester:
    """
    Service to ingest manuals from web URLs, specifically optimized for manuals.co.
    """
    ...
    async def extract_pdf_url(self, viewer_url: str) -> str | None:
        ...
        Handles Nitro-lazy-loading attributes.
        """
        async with httpx.AsyncClient(
            headers=self.headers, follow_redirects=True, timeout=30.0
        ) as client:
            try:
                ...
                if iframe:
                    ...
                    if "viewer.php?file=" in pdf_url:
                        pdf_url = pdf_url.split("viewer.php?file=")[-1].split("&")[0]
    ...
    async def download_pdf(self, pdf_url: str, filename: str) -> Path:
        """
        Downloads a PDF from a URL to the local upload directory.
        """
    ...
    async def scrape_with_jina(self, url: str) -> str | None:
        """
        Uses r.jina.ai to convert a URL to clean markdown.
        """
    ...
    async def scrape_charm_li(self, url: str, vehicle_context: str, limit: int = 50) -> int:
        """
            ...
            while to_visit and processed_count < limit:
                ...
                try:
                    ...
                    for a in links:
                        ...
                        if href.startswith("/") and branch_prefix in f"https://charm.li{href}":
                    ...
                    # Only process pages that look like terminal content (not just navigation)
                    # Navigation pages usually have many links but little text
                    ...
                    await asyncio.sleep(1) # RPM compliance
    ...
    async def process_url(self, url: str, vehicle_context: str):
        ...
        try:
            ...
            if any(url.lower().endswith(ext) for ext in [".jpg", ".jpeg", ".png"]):
                ...
                filename = url.split("/")[-1]
                local_path = await self.download_pdf(url, filename)
                # For images, we index the metadata and a placeholder or OCR if available
                # Here we just index the link and context as high-fidelity visual data
                doc = Document(
                    text=f"TECHNICAL IMAGE: {vehicle_context} - {filename}\nSource: {url}",
                    metadata={
                        "source": url,
                        "vehicle_context": vehicle_context,
